...
// disabled: true → not selectable in the builder (not wired end-to-end)
export const TRIGGER_STYLES: Record<string, TypeStyle> = {
  ...
  "New Email Received": {
    ...
    icon: Mail,
    disabled: true,
    disabledReason: "Coming soon — needs inbound email provider",
  },
  "New Row in Spreadsheet": {
    ...
    disabled: true,
    disabledReason: "Coming soon — needs Google Sheets OAuth + poll",
  },
  "New File in Drive": {
    bg: "bg-yellow-500/20",
    ...
    icon: FolderOpen,
    disabled: true,
    disabledReason: "Coming soon — needs Google Drive OAuth + poll",
  },
};

/** Triggers that are fully wired (fallback if API omits ready flags) */
export const READY_TRIGGER_NAMES = Object.entries(TRIGGER_STYLES)
...
export const ACTION_STYLES: Record<string, TypeStyle> = {
  ...
  "Create Spreadsheet Row": {
    ...
    icon: FileSpreadsheet,
    disabled: true,
    disabledReason: "Needs Google Sheets connection (OAuth)",
  },
  "Send SMS": {
    bg: "bg-green-500/20",
    text: "text-green-500",
    icon: Phone,
    disabled: true,
    disabledReason: "Needs Twilio — not configured",
  },
};
...
export function isTriggerDisabled(name: string): boolean {
  return TRIGGER_STYLES[name]?.disabled ?? false;
}

/** Prefer API ready/comingSoon; fall back to local style map */
export function isTriggerSelectable(opts: {
  name: string;
  ready?: boolean;
  comingSoon?: boolean;
}): boolean {
  if (opts.ready === true) return true;
  if (opts.ready === false || opts.comingSoon === true) return false;
  return !isTriggerDisabled(opts.name) && READY_TRIGGER_NAMES.includes(opts.name);
}
